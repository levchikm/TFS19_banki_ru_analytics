...
#### 2. Поиск тематик (Лев, Никита, Артем)
  ...
  Во вложениях добавлены примеры файлов, где показана работа алгоритма на предложениях (не на целых отзывах), а также есть файлы со сравнительным анализом между группами банков. Чтобы удобнее было это смотреть, лучше перейти по ссылке https://drive.google.com/open?id=1qWrIhE6sOTO5tlwpR2BUBToXZ-2jVWGB , чтобы посмотреть работу алгоритма на отдельных банках или по этой ссылке https://drive.google.com/open?id=1WfX91lfoEyRtASYeTzcNWYAYo5jOCaQx , чтобы посмотреть сравнительный анализ для нескольких групп банков. Файлы, с помощью которых загружалась тональность и проводились все расчеты, располагаются в репозитории (это файлы с длинными поясняющими названиями)
